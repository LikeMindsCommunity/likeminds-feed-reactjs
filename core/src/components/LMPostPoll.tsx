...
import {
  getTimeLeftInPoll,
  hasPollEnded,
  getPollSelectionText,
  PollOption,
  multipleOptionSubmitVoteValidation,
  isInstantPoll,
} from "../shared/utils";
...
export const LMPostPoll = () => {
  ...
  const pollSelectionText = getPollSelectionText(
    multiSelectNo,
    multiSelectState,
  );
        ...
        <div key={index}>
          <div
            ...
            className={
              ...
              (pollOption.isSelected
                ? ` poll-feed-selected-option-wrapper `
                : ` `) +
              (hasPollEnded(pollExpiryTime)
                ? `cursor-default poll-margin-remove`
                : ``) +
              (pollData?.lmMeta?.toShowResults ? ` ` : ` poll-results-margin `)
            }
          >
            {((hasSelectedOption && isInstantPoll(pollType)) ||
              ...
              <div
                ...
                style={{
                  width: `${pollData?.lmMeta?.toShowResults ? pollOption.percentage : 0}%`,
                }}
              />
            )}
            <div
              className={
                ...
                (((hasSelectedOption && isInstantPoll(pollType)) ||
                  hasPollEnded(pollExpiryTime)) &&
                !pollOption.isSelected
                  ? ` post-poll-unselected-option-custom-style `
                  : ` `)
              }
            >
              {pollOption.text}
          ...
          {((isInstantPoll(pollType) && hasSelectedOption) ||
            ...
            pollData?.lmMeta?.toShowResults && (
              <div
                onClick={() => {
                  onOptionVoteCountClick(index, true);
                }}
  ...
  return (
    <div className="attachment-poll">
      ...
      {pollSelectionText && (
        <div className="poll-feed-vote-count poll-feed-preview-advance-options-select poll-preview-subheading-style post-poll-info-custom-style">
          {pollSelectionText}
        </div>
      )}
      ...
      {showAddOptionButton ? (
        <div
          className="poll-feed-option-add-more post-poll-add-option-custom-style"
          onClick={() => {
      ...
      {showSubmitVoteButton &&
        (multipleOptionSubmitVoteValidation(
          ...
          <div className="lm-feed-create-post-wrapper__submit-button lm-mt-4 poll-feed-multiple-options-submit-button poll-feed-submit-button-disabled post-poll-submit-button-custom-style">
            Submit Vote
          </div>
        ))}
      ...
      <div className="poll-feed-vote-count poll-feed-preview-advance-options-total poll-preview-subheading-style">
        {((isInstantPoll(pollType) && hasSelectedOption) ||
          hasPollEnded(pollExpiryTime)) && (
          <div
            className="lm-cursor-pointer poll-feed-total-votes member-voted-count-custom-style"
            onClick={() => {
              setPollResultSelectedTabFunction(0);
              setResultScreenDialogOpenFunction(true);
            }}
          >
            {totalMembersVotes <= 1
              ? `${totalMembersVotes} vote`
              : `${totalMembersVotes} votes`}
          </div>
        )}
        {!hasSelectedOption &&
          !hasPollEnded(pollExpiryTime) &&
        ...
        {!hasPollEnded(pollExpiryTime) &&
          ...
          !isEditMode && (
            <div
              className="lm-cursor-pointer poll-feed-total-votes"
              onClick={() => {
                setIsEditModeFunction(true);
              }}
            >
      ...
      >
        <div className="lm-feed-create-post-wrapper">
          ...
          <div className="poll-feed-add-option-dialog-text">
            Enter an option that you think is missing in this poll. This cannot
            be undone.
          </div>
          ...
          <div
            className={
              `lm-cursor-pointer lm-feed-create-post-wrapper__submit-button lm-mt-4 poll-feed-submit-button ` +
              (newOption.trim().length === 0
                ? `poll-feed-submit-button-disabled`
                : ``) +
              ` poll-feed-add-poll-option-submit-button-custom-style `
            }
      ...
      ((isInstantPoll(pollType) && pollData?.lmMeta?.toShowResults) ||
        hasPollEnded(pollData?.metadata.expiryTime)) ? (
        <Dialog
        ...
        >
          <div className="lm-feed-create-post-wrapper lm-feed-poll-wrapper2 ">
            ...
            <div className="poll-results-screen-wrapper">
                    ...
                    <Tab
                      label={
                        <div
                          className={
                            `poll-results-tab2 ` +
                            (pollResultSelectedTab == idx
                              ? ` post-poll-results-selected-option-custom-style `
                              : ` post-poll-results-other-option-custom-style `)
                          }
                        >
                          {" "}
              ...
              <div
                className="poll-results-memeber-list-wrapper"
                id="poll-scroll-container"
              >
        ...
        >
          <div className="lm-feed-create-post-wrapper">
            ...
            {pollData?.metadata.isAnonymous ? (
              <div className="poll-feed-add-option-dialog-text">
                This being an anonymous poll, the names of the voters cannot be
                disclosed.
              </div>
            ) : (
              <div className="poll-feed-add-option-dialog-text">
