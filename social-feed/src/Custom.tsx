    ...
    <>
      ...
      <div className="lm-createPost">
        <div className="lm-createPost__media">
          <div className="lm-createPost__media__imgBox lm-avatar">{avatar}</div>
          <div
            onClick={() => {
              setOpenCreatePostDialog(true);
            }}
...
const CustomLMFeedCreatePostDialog = (prop: LMFeedCreatePostDialogProps) => {
  ...
  const widgets: any = undefined;
  const temp = useContext(LMFeedDataContext as any) as any;
  console.log(temp);
  const metaInputRef = useRef(null);
  const attachmentId = useRef(null);
  const { CustomComponents = {} } = useContext(
    CustomAgentProviderContext as any
  ) as any;
  const { CustomTopicDropDown } = CustomComponents;
  useEffect(() => {
    if (attachmentId)
      attachmentId.current = temporaryPost?.attachments?.find(
        (attachment: any) => attachment?.attachmentType === 5
      )?.attachmentMeta?.entityId;
  }, [temporaryPost]);
  useEffect(() => {
    if (widgets && metaInputRef?.current && attachmentId?.current) {
      (metaInputRef.current as any).value =
        widgets?.[attachmentId.current]?.metadata?.widgetMeta?.meta?.metaData ||
        "";
    }
  }, [widgets, metaInputRef, attachmentId]);

  useEffect(() => {
    document.addEventListener("OPEN_CREATE_POST_DIALOUGE", (e) => {
      console.log("OPEN_CREATE_POST_DIALOUGE", e);
    });
  });
